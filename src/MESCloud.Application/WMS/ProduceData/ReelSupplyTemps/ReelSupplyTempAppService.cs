...
{
    public class ReelSupplyTempAppService : AsyncCrudAppService<ReelSupplyTemp, ReelSupplyTempDto, string, PagedResultRequestMESDto, ReelSupplyTempDto, ReelSupplyTempDto>, IReelSupplyTempAppService
    {
        readonly IRepository<ReelSupplyTemp, string> _repository;
        readonly IRepository<ReadyMBill, string> _repositoryrb;
        readonly IRepository<ReadyMBillDetailed, string> _repositoryrbd;
        ...
        public ReelSupplyTempAppService(
            IRepository<ReelSupplyTemp, string> repository,
            IRepository<ReadyMBill, string> repositoryrb,
            IRepository<ReadyMBillDetailed, string> repositoryrbd,
        ...
        [HttpPost]
        public async override Task<PagedResultDto<ReelSupplyTempDto>> GetAll(PagedResultRequestMESDto input)
        {
            var query = MESPagedResult.GetMESPagedResult<ReelSupplyTemp>(input, _repository.GetAll());
            ...
            return new PagedResultDto<ReelSupplyTempDto>(tasksCount, taskList.MapTo<List<ReelSupplyTempDto>>());
        }

        public async Task<ICollection<string>> GetPartNoIdsByKeyName(string readyBill, string keyName)
        {
            //// 查询当前备料单详细
            //var readyNow = await _repositoryrb.FirstOrDefaultAsync(readyBill);

            //if (readyNow == null)
            //{

            //    throw new MesException("备料单[" + readyBill + "]不存在");
            //}
        ...
        public async Task<ReelSupplyResultDto> Supply(ReelSupplyInputDto[] input)
        {
            ...
            // 亮灯
            var lights = _repositorySL.GetAllList(s => s.IsBright);
