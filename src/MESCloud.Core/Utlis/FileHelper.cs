...
namespace MESCloud.Utils
{

    public class FileHelperService
    {

        public async Task<List<List<T>>> ExcleToListEntities<T>(List<I18N> i18ns, FileStream fileStream) where T : class, new()
        {
                ...
                using (ExcelPackage package = new ExcelPackage(fileStream))
                {
                    foreach (var sheet in package.Workbook.Worksheets)
                    {
                        try
                        {
                            ...
                            Dictionary<int, I18N> columnInfos = new Dictionary<int, I18N>();
                            // 读取表头
                            for (int j = sheet.Dimension.Start.Column, k = sheet.Dimension.End.Column; j <= k; j++)
                            {
                                var columnName = sheet.Cells[sheet.Dimension.Start.Row, j].Value.ToString();
                                // 查询是否包含此列
                                var columnInfo = i18ns.Where(i => i.DisplayName == columnName).FirstOrDefault();
                                if (columnInfo != null)
                                {
                            ...
                            for (int m = sheet.Dimension.Start.Row + 1, n = sheet.Dimension.End.Row; m <= n; m++)
                            {
                                ...
                                for (int j = sheet.Dimension.Start.Column, k = sheet.Dimension.End.Column; j <= k; j++)
                                {
                                    ...
                                    if (columnInfo.Value != null)
                                    {
                                        ...
                                        info = objType.GetProperty(columnInfo.Value.PropertyName);
                                        //判断对象是否有该属性
                                        if (info != null)
                                        {
                                            //为对象属性赋值
